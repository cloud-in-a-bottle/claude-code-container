...
- each workspace is billed either to an `ANTHROPIC_API_KEY` or to the user's claude subscription, fixed when the workspace was created; the sidebar gear opens settings, where the subscription is set up with `claude auth login`. in a subscription workspace there is no api key in the environment, and that is deliberate — don't export one to "fix" it.
- images pasted into a terminal are uploaded by the page and land in `~/.workbench/pasted-images/`, and the path is pasted into the prompt for you. your own clipboard paste can't work here — it reads the container's clipboard, not the user's — so if someone reports "no image in clipboard", tell them to paste into the terminal itself.
- `linear` is on PATH and talks to Linear with the owner's credentials injected by the latchkey app, so there is no api key here to find or to ask for. `linear comment ENG-123 "..."` posts on an issue; `linear --help` has the rest. a workspace opened from a Linear issue is expected to use it to report back when its PR is up.
